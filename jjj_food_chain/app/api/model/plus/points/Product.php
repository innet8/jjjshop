...
class Product extends ProductModel
{
    ...
    public function getProductList($params)
    {
        ...
        $list = $model->where('product_status', '=', 10)
            ->where('is_delete', '=', 0)
            ->with(['image'])
            ->paginate($params);
        return $list;
    ...
    public function getProductOrder($params, $user)
    {
        ...
        // 商品支持的配送方式
        $deliveryType = $detail['delivery_set'];
        sort($deliveryType);
        $detail['delivery_set'] = $deliveryType;
        $delivery = $params['delivery'] > 0 ? $params['delivery'] : $deliveryType[0];
        ...
        $detail['shop_supplier_id'] = isset($params['shop_supplier_id']) ? $params['shop_supplier_id'] : 0;
