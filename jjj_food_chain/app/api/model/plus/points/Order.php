...
class Order extends OrderModel
{
    ...
    private function add($order, $user)
    {
        ...
        if ($order['delivery'] == DeliveryTypeEnum::EXTRACT) {
            $data['shop_supplier_id'] = $order['shop_supplier_id'];
        }
        //随主订单配置
    ...
    {
        $model = new static();
        $order = $model->where(['order_id' => $order_id, 'user_id' => $user_id,])->with(['product', 'user', 'image', 'store', 'express'])->find();
        if (empty($order)) {
