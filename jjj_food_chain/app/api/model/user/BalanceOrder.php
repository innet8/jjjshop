...
class BalanceOrder extends BalanceOrderModel
{
    ...
    public function createOrder($user, $plan_id, $user_money)
    {
        ...
        if (!$settings['is_open']) {
            throw new BaseException(['msg' => '余额充值未开启']);
        }
        if ($plan_id <= 0 && $user_money < $settings['min_money']) {
            throw new BaseException(['msg' => '最低充值金额' . $settings['min_money']]);
        }
        // 获取订单数据
        $data = [
    ...
    public static function getPayDetail($orderNo, $pay_status)
    {
        ...
        return $model->where(['order_no' => $orderNo])->with(['user'])->find();
    }
    ...
    {
        $order['trade_no'] = $order['order_no'];
        $order['online_money'] = $order['pay_price'];
