...
class Cart extends CartModel
{
    ...
    public function getOrderCartDetail($cashier, $table_id, $order_id = 0)
    {
        ...
        if ($order) {
            ...
            $order['buffet_remaining_time'] = Order::getBuffetRemainingTime($order['order_id'], $start_timestamp);
        } else {
            $order_total_num = 0;
            $order_total_price = 0;
    ...
    public function preOrderCartPrice($shop_supplier_id, $mobile, $table_id, $order_id)
    {
        ...
        // 用户信息
        $user = (new UserModel)->where('mobile', $mobile)->whereOr('user_id', $mobile)->where(['is_delete' => 0])->with(['grade', 'card', 'cardRecord'])->find();
        if (!$user) {
            $this->error = "用户不存在";
            return false;
        }
