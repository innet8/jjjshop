      ...
      <div class="table-wrap">
        <el-table size="small" :data="tableData" style="width: 100%;" row-key="access_id" border default-expand-all  v-loading="loading">
            ...
            <el-table-column prop="weixin_service" label="微信服务商支付">
              <template #default="scope">
                <el-checkbox v-model="scope.row.weixin_service" @change="checked => wxStatusChange(checked, scope.row)">
                  启用</el-checkbox>
              </template>
            </el-table-column>
            <el-table-column prop="is_chain" label="连锁店">
              <template #default="scope">
                <el-checkbox v-model="scope.row.is_chain" @change="checked => ChainChange(checked, scope.row)">
                  启用</el-checkbox>
              </template>
            </el-table-column>
  ...
  export default {
    ...
    methods: {
        ...
        ShopApi.shopList({
          ...
          .then(data => {
            ...
            self.tableData.forEach(function(message) {
              ...
              message.is_chain = message.is_chain === 1 ? true : false;
            });
          })
          .catch(error => {});
      },
      ...
      editClick(e) {
        ...
        this.curModel.is_chain = e.is_chain === true ? 1 : 0;
      ...
      ChainChange: function(checked, row) {
        let self = this;
        self.loading = true;
        ShopApi.updateChain({
              app_id: row.app_id
