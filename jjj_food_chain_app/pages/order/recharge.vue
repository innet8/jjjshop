<template>
	<view class="index" v-if="!loading"  :data-theme='theme()' :class="theme() || ''">
		<view class="index-head">
			<view class="index-head-top">
				<view class="f32">请选择充值套餐:</view>
			</view>
			<view class="index-head-bottom">
				<view class="index-head-bottom-item" @click="select(index)" v-for="(item,index) in tableData" :key="index">
		...
		<view class="index-body" v-if="settings.is_plan == 1">
		...
		<view class="btn">
			<button class="theme-btn" type="default" @click="payFunc">确认支付</button>
		</view>
	</view>
</template>
