				...
				<view class="other_box">
					...
					<view class="item" v-if="OrderData.is_coupon">
						...
						<block v-if="coupon_num > 0">
							<text class="vlaue gray9" v-if="OrderData.coupon_id > 0"
								@click="onTogglePopupCoupon(coupon_list)">-￥{{ OrderData.coupon_money }}</text>
							<text v-else class="vlaue gray9"
								@click="onTogglePopupCoupon(coupon_list)">有{{ coupon_num }}张优惠券可用</text>
						</block>
						<text v-else class="vlaue gray9">无优惠券可用</text>
					</view>
		...
		methods: {
			...
			getData() {
				...
				let callback = function(res) {
					...
					self.coupon_id = self.OrderData.coupon_id;
