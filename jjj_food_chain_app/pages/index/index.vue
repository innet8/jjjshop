		...
		getData() {
			...
			self._post(
				...
				function(res) {
					...
					if (isFirst == '' && res.data.data.setting.collection.status == '1') {
						self.is_collection = true;
						uni.setStorageSync('isFirst', 1);
					}
					self.is_follow = res.data.data.setting.officia.status;
					// #endif
					// 首页推送
					let homepush_name = uni.getStorageSync('homepush_name');
					if (res.data.data.setting.homepush.is_open && homepush_name != res.data.data.setting.homepush.name) {
						self.homepush_data = res.data.data.setting.homepush;
