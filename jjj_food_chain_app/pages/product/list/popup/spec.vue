<template>
	<view>
		<!-- 商品详情模态框 begin -->
		<modal :show="goodDetailModalVisible" class="good-detail-modal" color="#5A5B5C" width="100%" custom padding="0rpx" radius="0" @cancel="closeGoodDetailModal">
			<view class="cover"><image v-if="form.detail.product_image" :src="form.detail.product_image" class="image" mode="aspectFill"></image></view>
			<view>
				...
				<view class="wrapper">
					<view class="properties">
						<view class="property" v-if="form.detail.spec_type == 20">
							...
							<view class="values">
								<view class="value" @click="selecedtSpec(value, key)" v-for="(value, key) in form.detail.sku" :key="key" :class="{ default: value.checked }">
									{{ value.spec_name }}
								</view>
							</view>
						</view>
						<view
							class="property"
							v-if="form.detail.product_attr && form.detail.product_attr.length > 0"
							v-for="(item, index) in form.detail.product_attr"
							:key="index"
						>
							<view class="title">
						...
						<view class="property" v-if="form.detail.product_feed && form.detail.product_feed.length > 0">
							<view class="title"><text class="name">加料</text></view>
							<view class="values">
								<view
									class="value"
									@click="selectFeed(item, index)"
									v-for="(item, index) in form.detail.product_feed"
									:key="index"
					...
					<view class="d-c d-a-s">
						<view class="left">
							<view class="price">
								...
								<text class="fb">{{ price }}</text>
								<text class="f22 gray9 text-l-t">￥{{ lineprice }}</text>
								<text class="fb"></text>
								<text v-if="bag_type != 1">
									<text class="f24 gray9">(￥{{ bag_price }})</text>
								</text>
							</view>
						</view>
...
export default {
	...
	props: ['goodDetailModalVisible', 'productModel', 'orderType', 'bag_type', 'dinner_type', 'cart_type', 'table_id'],
	computed: {
		bag_price: function() {
		...
		lineprice: function() {
			return (this.feed_price * 1 + this.product_lineprice * 1) * this.form.show_sku.sum;
		},
		/*判断增加数量*/
		isadd: function() {
	...
	methods: {
		...
		addCart() {
			...
			if (this.$props.table_id) {
				params.table_id = this.$props.table_id;
			}
			self._post(
				'order.cart/add',
				params,
				function(res) {
					...
					let price = res.data.cartInfo.total_price;
					let total_bag_price = res.data.cartInfo.total_bag_price;
					self.clock = false;
					self.$emit('closeGoodDetailModal', self.form.show_sku.sum, res);
				},
				() => {
					self.clock = false;
				}
