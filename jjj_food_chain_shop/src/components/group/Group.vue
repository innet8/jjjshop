<template>
	<!--
    	...
    	描述：组件-选择商品
    -->
	<el-dialog title="选择团购" v-model="dialogVisible" @close="dialogFormVisible" :close-on-click-modal="false"
				...
				<el-table size="small" :data="productList" border style="width: 100%" highlight-current-row
					...
					<el-table-column width="70" label="团购图片">
						<template #default="scope">
							<img :src="scope.row.image && scope.row.image[0] && scope.row.image[0].file_path"
								:width="30" :height="30" />
						</template>
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			getData() {
				...
				GroupApi.groupListData(params, true)
					.then(res => {
						callback(res)
					})
