<template>
	<div
		class="drag optional "
		:style="{
			background: item.style.bgcolor,
			paddingLeft: item.style.paddingLeft + 'px',
			paddingRight: item.style.paddingLeft + 'px',
			paddingTop: item.style.paddingTop + 'px',
			paddingBottom: item.style.paddingBottom + 'px'
		}"
		:class="{ selected: index === selectedIndex }"
		@click.stop="$parent.$parent.onEditer(index)"
	>
		<div
			class="diy-sharpproduct"
			:style="{
				background: item.style.background,
				borderTopLeftRadius: item.style.topRadio + 'px',
				borderTopRightRadius: item.style.topRadio + 'px',
				borderBottomLeftRadius: item.style.bottomRadio + 'px',
				borderBottomRightRadius: item.style.bottomRadio + 'px'
			}"
		>
    	<div class="sharpproduct-head d-b-c"	>
				<div class="left d-s-c">
					<div class="name">
						推荐商品
					</div>
				</div>
				<div class="right gray d-s-c" style="line-height: 1;"	>
         	 <div class="name2">
					查看全部</div>
				</div>
			</div>
			<ul class="product-list column__3" :style="getUlwidth(item)">
				<li
					class="product-item"
					:style="{
						background: item.style.productBg_color,
						borderBottomLeftRadius: item.style.product_bottomRadio + 'px',
						borderBottomRightRadius: item.style.product_bottomRadio + 'px',
						borderTopLeftRadius: item.style.product_topRadio + 'px',
						borderTopRightRadius: item.style.product_topRadio + 'px'
					}"
					v-for="(product, index) in item.data"
					:key="index"
				>
					<!-- 两列三列 -->
					<div class="product-cover pr">
						<img :style="{ borderRadius: item.style.product_imgRadio + 'px' }" src="/src/assets/img/login_logo.png" />
						<div
							:style="{
								color: item.style.salesColor,
								background: item.style.bgSales
							}"
						
							class="product-sales tl"
						>
							<text class="text">摩尔城店</text>
						</div>
					</div>
					<div class="product-info p-0-10">
						<div class="f14 fb text-ellipsis ">
							<div
								class="f12 tl text-ellipsis"
								:style="{
									color: item.style.productName_color
								}"
							>
								商品名称
							</div>
               <div class="sharpproduct-head d-b-c">
              <div >
              <div class="f14 tl price"	:style="{	color: item.style.productPrice_color	}">
								¥120
							</div>
							<div :style="{	color: item.style.productLine_color}"	class=" lineprice1 text-d-line">
								¥233
							</div>
         
              </div>
					  	   <div class="assemble_btns  ">
							抢购
						</div>
            </div>
						</div>
					</div>
				</li>
			</ul>
		</div>
		<div class="btn-edit-del"><div class="btn-del" @click.stop="$parent.$parent.onDeleleItem(index)">删除</div></div>
	</div>
</template>

<script>
export default {
	data() {
		return {
			/*商品列表*/
			tableData: [],
			/*分类id*/
			category_id: 0
		};
	},
	created() { },
	props: ['item', 'index', 'selectedIndex'],
	methods: {
		/*计算宽度*/
		getUlwidth(item) {
			if (item.style.display == 'slide') {
				let total = 0;
				if (item.params.source == 'choice') {
					total = item.data.length;
				} else {
					total = item.defaultData.length;
				}
				let w = 0;
				if (item.style.column == 2) {
					w = total * 150;
				} else {
					w = total * 100;
				}
				return 'width:' + w + 'px;';
			}
		}
	}
};
</script>

<style lang="scss" scoped>
.lineprice1{
   color: #999999;
   font-size: 10px;
   margin-bottom: 20px;
 
}
.assemble_btns{
  font-size: 12px;
  color: #FFFFFF;
	width: 40px;
	height: 25px;
	line-height: 1;
	display: flex;
	justify-content: center;
  font-weight: 200;
	align-items: center;
  border-radius: 15px;
  margin-bottom: 10px;
  background-color: #FFCC00;
  margin-right: 10px;
}

.diy-sharpproduct {
	overflow: hidden;
  border-radius: 7px;
	// padding: 10px;
}

.diy-sharpproduct .product-list {
	display: flex;
	justify-content: flex-start;
	flex-wrap: wrap;
	padding-top: 11px;
	padding-left: 10px;
	box-shadow: 0px 4px 2px 0px rgba(6, 0, 1, 0.03);
	padding-bottom: 10px;
}

.diy-sharpproduct .display__list .column__3 {
	display: flex;
	flex-wrap: wrap;
	justify-content: space-between;
	padding: 10px;
}

.diy-sharpproduct .product-list.column__3 .product-item {
	width: 113px;
	margin-right: 10px;
	padding-bottom: 5px;
	overflow-x: hidden;
}
.diy-sharpproduct .product-list.column__3 .product-item:last-child {
	margin-right: 0;
}
.diy-sharpproduct .product-list.column__3 .product-item .product-cover {
	width: 99px;
	height: 99px;
	position: relative;
	overflow: hidden;
	margin-bottom: 6px;
  border-radius: 10px;
}

.diy-sharpproduct .product-list .price {
	margin-bottom: 5px;
  margin-top: 7px;
  font-size: 13px;
	width: 100%;
	display: -webkit-box;
	overflow: hidden;
  color: #FF5800;
  font-weight: 200;
	-webkit-line-clamp: 1;
	-webkit-box-orient: vertical;
	text-align: start;
}
.diy-sharpproduct .product-list.column__3 .product-item .product-cover img {
	width: 120px;
	height: 99px;
  border-radius: 10px;
}
.diy-sharpproduct .sharpproduct-head {
	height: 34px;
	display: flex;
	justify-content: space-between;
	align-items: center;
	box-sizing: border-box;
	margin: 0;
  padding-top: 16px;
	width: 100%;
	background-repeat: no-repeat;
	background-size: cover;
	line-height: 1;
}

.sharpproduct-head .name {
	font-size: 18px;
	font-weight: bold;
	width: 100px;
  padding-left: 11px;
}
.sharpproduct-head .name2 {
padding-right: 10px;
color: #666666;
}
.sharpproduct-head .datetime {
	margin-left: 20px;
}

.sharpproduct-head .datetime > span {
	display: inline-block;
}

.sharpproduct-head .datetime .text {
	padding: 0 2px;
  color: #FFFFFF;
}

.diy-sharpproduct .product-list .barginbtns {
	width: 100px;
	background: linear-gradient(90deg, #ff4545 0%, #f6220c 100%);
	height: 25px;
	line-height: 25px;
	font-size: 13px;
	text-align: center;
	color: #ffffff;
	margin: 0 auto;
	border-radius: 13px;
}

.left .datetime {
	padding: 1px;
	border-radius: 200px 0 200px 200px;
	border: 1px solid #eeeeee;
	padding-right: 4px;
}

.left .datetime .daybox {
	padding: 0 10px;
	border-radius: 200px;
	margin-right: 5px;
}
.titleImg {
	width: auto;
	height: 44px;
}
.product-info {
	height: auto;
  text-align: start;
  padding-top: 2px;
  padding-left: 5px;
}
.product-sales .text{
  color: #ffffff;
  padding-left: 18px;
  font-weight: 200;
}
.product-sales {
	font-size: 10px;
	width: 71px;
	line-height: 1.7;
	position: absolute;
	bottom: 0;
	left: 0;
	right: 0;
background: #000000;
opacity: 0.65;
border-radius: 0px 12px 0px 0px;
  white-space: nowrap;
}
</style>
