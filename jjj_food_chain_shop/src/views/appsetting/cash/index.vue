<template>
    <div class="cash">
            ...
            <el-form size="small" ref="form" :model="form" label-position="top">
                <el-form-item :label="$t('轮播内容')" prop="carousel" :rules="[{ required: true, message: '' }]">
                    <div class="draggable-list">
                        <flieUpload @upLoad="upLoad" :tips1="$t('图片：支持JPG、JPEG、PNG格式，小于15MB，尺寸：1024*600px')" :tips2="$t('视频：支持AVI、MPEG、MOV、MP4格式，小于30MB，尺寸：1024*600px')">
                        </flieUpload>
        ...
        <setPassword v-if="open" :open="open" :openTitle="openTitle" :cashierPassword="form.cashier_password" @close="(e) => { open = false; if (e == 1) { this.getData(); } }">
        </setPassword>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        onSubmit() {
            ...
            self.loading = true;
            Terminal.saveTerminal(params, true).then(data => {
                self.loading = false;
                this.$ElMessage({
                    message: $t('保存成功'),
                    type: 'success'
