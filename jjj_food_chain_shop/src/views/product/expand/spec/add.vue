    ...
    <el-dialog :title="$t('添加规格')" v-model="dialogVisible" @close="dialogFormVisible" :close-on-click-modal="false"
        ...
        <el-form size="small" :model="form" label-position="top" :rules="formRules" ref="form">
            ...
            <template v-for="(item, index) in languageList" :key="index">
                <el-form-item :label="$t('规格名称') + `(${item.value})`" :prop="`spec_name.${item.key}`">
                    <el-input v-model="form.spec_name[item.key]" :placeholder="$t('请输入规格名称')" :maxlength="50"
                        autocomplete="off"></el-input>
                </el-form-item>
            </template>
            <el-form-item :label="$t('规格排序')" prop="sort">
                <el-input-number :controls="false" :precision="0" :min="0" :max="999" :placeholder="$t('接近0，排序等级越高')" v-model.number="form.sort" autocomplete="off"></el-input-number>
            </el-form-item>
