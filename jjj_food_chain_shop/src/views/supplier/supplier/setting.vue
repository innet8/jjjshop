<template>
  <div class="pb50">
    <el-form size="small" ref="form" :model="form" label-width="150px">
      ...
      <el-form-item label="店内堂食结算模式">
        <el-radio-group v-model="form.settle_type">
          <el-radio :label="10">先结账后用餐</el-radio>
          <el-radio :label="20">先用餐后结账</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="收银台桌台清台设置" v-if="form.settle_type==10">
        <el-radio-group v-model="form.auto_close">
          <el-radio :label="0">定时清台</el-radio>
          <el-radio :label="1">立即清台</el-radio>
        </el-radio-group>
        <div class="tips">开启定时清台后，系统将在设置的时间后自动清除收银台点餐的桌台，此功能仅在餐前付款模式下生效</div>
      </el-form-item>
      <el-form-item  :rules="[{ required: true, message: ' ' }]" prop="close_time" label="桌台订单清台设置">
        <div class=" d-s-c">
          <div class="max-w60">
            <el-input v-model="form.close_time" type="number"></el-input>
          </div>
          <div class="ml10">分钟后自动清理桌台（扫桌码点餐和收银桌台点餐定时清台时间）</div>
        </div>
      </el-form-item>
      <el-form-item label="店内堂食服务费">
        <el-radio-group v-model="form.serviceType">
          <el-radio :label="0">按就餐人数收取</el-radio>
          <el-radio :label="1">按每桌台收取</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item v-if="form.serviceType==0" label="" :rules="[{ required: true, message: '请输入服务费' }]"
        prop="bag_price">
        <div class=" d-s-c">
          <div class="max-w60">
            <el-input v-model="form.service_money" type="number"></el-input>
          </div>
          <div class="ml10">元/位</div>
        </div>
      </el-form-item>
      <el-form-item v-if="form.serviceType==1" label="" :rules="[{ required: true, message: '请输入服务费' }]"
        prop="bag_price">
        <div class=" d-s-c">
          <div class="max-w60">
            <el-input v-model="form.service_money" type="number"></el-input>
          </div>
          <div class="ml10">元/桌</div>
        </div>
      </el-form-item>
