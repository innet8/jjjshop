<template>
    <!--
      ...
      描述：会员-等级管理-编辑等级
  -->
    <el-dialog :title="$t('编辑等级')" v-model="dialogVisible" @close='dialogFormVisible' :close-on-click-modal="false"
        :close-on-press-escape="false" width="600px">
        <el-form size="small" :model="form" label-position="top" ref="form">
            <el-form-item :label="$t('等级名称')" :label-width="formLabelWidth" prop="name"
                :rules="[{ required: true, message: $t('请输入等级名称') }]">
                <el-input v-model="form.name" :placeholder="$t('请输入等级名称')"></el-input>
            </el-form-item>
            <el-form-item :label="$t('等级权重')" :label-width="formLabelWidth" prop="weight"
                :rules="[{ required: true, message: $t('请输入等级权重') }]">
                <el-input v-model="form.weight" type="number" :placeholder="$t('请输入等级权重')"></el-input>
                <div class="gray9">{{ $t('权重越大，等级越高') }}</div>
            </el-form-item>
            <el-form-item :label="$t('等级折扣')" :label-width="formLabelWidth" prop="equity"
                :rules="[{ required: true, message: $t('请输入等级折扣') }]">
                <el-input type="number" :precision="1" :step="1" :min="0" :max="100" :placeholder="$t('请输入等级折扣')"
                    v-model="form.equity">
                    <template #append>%</template>
                </el-input>
            </el-form-item>
            <el-form-item :label="$t('升级条件')" :label-width="formLabelWidth">
