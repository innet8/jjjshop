      ...
      <el-form ref="form" :model="form" :rules="formRules" label-width="150px">
        ...
        <el-form-item label="会员等级 " v-if="form.is_grade == 1">
          <el-select v-model="form.grade_ids" multiple placeholder="请选择">
            <el-option v-for="(item,index) in Grade" :key="index" :label="item.name" :value="item.grade_id+''">
            </el-option>
          </el-select>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        let self = this;
        let id = self.$route.query.package_id;
        PackageApi.getData({
          id: id
        }, true).then(data => {
      ...
      onSubmit() {
        ...
        self.$refs.form.validate((valid) => {
          if (valid) {
            ...
            self.loading = true;
            PackageApi.EditPackage(form, true).then(data => {
                self.loading = false;
                if (data.code == 1) {
                  ElMessage({
                    message: data.msg,
